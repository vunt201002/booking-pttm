...
import { uploadByLinkRoute, host, uploadFromDeviceRoute } from "../utils/apiRoute";
...
const PhotosUploader = ({ addedPhotos, onChange }) => {
    ...
    const addPhotoByLink = async (e) => {
        e.preventDefault();
        const res = await axios.post(uploadByLinkRoute, {
            link: photoLink
        });
    ...
    const uploadPhotoFromDevice = async (e) => {
        ...
        const res = await axios.post(uploadFromDeviceRoute, data, {
            headers: {
    ...
    const removePhto = (e, link) => {
        e.preventDefault();
        onChange([...addedPhotos.filter(photo => photo !== link)]);
    };

    const setlectAsMainPhoto = (e, link) => {
        e.preventDefault();
            ...
            <div className='grid grid-cols-3 gap-2 md:grid-cols-4 lg:grid-cols-6 mt-2'>
                {addedPhotos.length > 0 && addedPhotos.map((link, index) => (
                    <div key={index} className='h-32 flex relative'>
                        <img
                            src={`${host}/uploads/${link}`} 
                            alt=""
                            className='rounded-2xl w-full object-cover'
                        />
                        <button
                            className="absolute bottom-1 right-1 text-white bg-black p-2 cursor-pointer bg-opacity-50 rounded-2xl"
                            onClick={(e) => removePhto(e, link)}
                        >
                        ...
                        <button
                            className="absolute bottom-1 left-1 text-white bg-black p-2 cursor-pointer bg-opacity-50 rounded-2xl"
                            onClick={(e) => setlectAsMainPhoto(e, link)}
                        >
                            {link === addedPhotos[0] ? (
